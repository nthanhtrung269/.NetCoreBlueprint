...
namespace Blueprint.Polly.WebApi
{
    public class WebApiClient
    {
        ...
        public Task<T> GetAsync<T>(string description, Uri requestUri, CancellationToken cancellationToken, string applicableProvider)
        {
            return CallApiAsync<T>(description, client => CheckForWebException(description, client.GetAsync(requestUri, cancellationToken)), applicableProvider);
        }
        ...
        public Task<TResponse> PutAsync<TRequest, TResponse>(string description,
            Uri requestUri,
            TRequest requestContent,
            CancellationToken cancellationToken,
            string applicableProvider)
        {
            return CallApiAsync<TResponse>(description,
                client => CheckForWebException(description, client.PutAsync(requestUri, SerializeAsJsonString(requestContent), cancellationToken)), applicableProvider);
        }

        public Task<T> DeleteAsync<T>(string description,
            Uri requestUri,
            CancellationToken cancellationToken,
            string applicableProvider)
        {
            return CallApiAsync<T>(description,
                client => CheckForWebException(description, client.DeleteAsync(requestUri, cancellationToken)), applicableProvider);
        }
        ...
        private async Task<T> CallApiAsync<T>(string description, Func<HttpClient, Task<HttpResponseMessage>> apiCall, string applicableProvider)
        {
            try
            {
                ...
                using (HttpResponseMessage response = await apiCall(_httpClient))
                {
                    ...
                    {
                        _logger.LogDebug($"Response from {applicableProvider}: " + valueString);
            ...
            {
                _logger.LogWarning($"HTTP call to {applicableProvider} timed out.", ex);
                throw new TimeoutRejectedException($"HTTP call to {applicableProvider} timed out.", ex);
            }
            catch (Exception ex)
            {
                _logger.LogInformation($"HTTP call to {applicableProvider} failed.", ex);
                throw;
            }
        }
