...
namespace Blueprint.AdapterDesignPattern.Services.WebApiClientServices
{
    public class WebApiClient
    {
        ...
        /// <param name="jsonSerializerSettings">The jsonSerializerSettings.</param>
        /// <returns>Task{TResponse}.</returns>
        public Task<TResponse> PutAsync<TRequest, TResponse>(string description, Uri requestUri, TRequest requestContent, CancellationToken cancellationToken, string providerName, JsonSerializerSettings jsonSerializerSettings = null)
        {
            return CallApiAsync<TResponse>(description,
                client => CheckForWebException(description, client.PutAsync(requestUri, SerializeAsJsonString(requestContent), cancellationToken)), providerName);
        }
        ...
        private async Task<T> CallApiAsync<T>(string description, Func<HttpClient, Task<HttpResponseMessage>> apiCall, string providerName)
        {
            try
            {
                ...
                using (HttpResponseMessage response = await apiCall(_httpClient))
                {
                    ...
                    {
                        Message = $"{providerName} HTTP request",
                        RequestDescription = $"{description}",
                        ProviderName = providerName,
                        ElapsedMilliseconds = timer.ElapsedMilliseconds.ToString(),
                        HttpStatusCode = (int)response.StatusCode,
                        HttpResponseMessage = response.StatusCode.ToString()
                    };

                    _logger.LogInformation("{@httpRequestInfo}", httpRequestInfo);
                }
